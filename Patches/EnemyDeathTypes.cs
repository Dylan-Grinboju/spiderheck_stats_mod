...
class DeathCubeDamagePatch
{
    static void Prefix(DeathCube __instance, Vector3 direction, LineRenderer laser)
    {
        try
        {
            ...
            if (EnemyReflectionCache.DeathCubeLaserColliderLayers is null || EnemyReflectionCache.DeathCubeBeamWidth is null)
            {
                return;
            }
...
class DeathRayDamagePatch
{
    static void Prefix(DeathRay __instance)
    {
        try
        {
            ...
            if (EnemyReflectionCache.DeathRayDamageLayers is null || EnemyReflectionCache.DeathRayBeamWidth is null ||
                EnemyReflectionCache.DeathRayBeamLength is null || EnemyReflectionCache.DeathRayPoint is null)
            {
                return;
            }
...
class EnergyBallDamagePatch
{
    static void Prefix(EnergyBall __instance, Collision2D other)
    {
        try
        {
            ...
            if (EnemyReflectionCache.EnergyBallDamageEnemies is null ||
                EnemyReflectionCache.EnergyBallIsEnemy is null ||
                EnemyReflectionCache.EnergyBallIsDamageable is null)
            {
                return;
            }
    ...
    static void Prefix(ForceField __instance, Collision2D other)
    {
        try
        {
            ...
            if (willCallDamage)
            {
                ...
                if (parentWeapon is not null && parentWeapon.owner is not null)
                {
...
class SpiderHealthSystemAfterlifeExplosionPatch
{
    static void Prefix(SpiderHealthSystem __instance)
    {
        try
        {
            if (EnemyReflectionCache.SpiderKnockBackRadius is null || EnemyReflectionCache.SpiderLayers is null || EnemyReflectionCache.SpiderDeathRadius is null)
            {
...
class ExplosionDamagePatch
{
    static void Prefix(Explosion __instance)
    {
        try
        {
            ...
            foreach (Collider2D collider2D in colliders)
            {
                ...
                if (componentInParent is null) continue;
    ...
    {
        if (EnemyReflectionCache.ExplosionKnockBackRadius is null || EnemyReflectionCache.ExplosionLayers is null || EnemyReflectionCache.ExplosionDeathRadius is null || EnemyReflectionCache.ExplosionPlayerDeathRadius is null || EnemyReflectionCache.ExplosionIsBoomSpear is null || EnemyReflectionCache.ExplosionPlayerExplosionID is null || EnemyReflectionCache.ExplosionOwnerId is null)
        {
...
class ProjectileLauncherShootPatch
{
    static bool Prefix(ProjectileLauncher __instance)
    {
        try
        {
            ...
            if (particleBladeComponent is null)
            {
            ...
            if (basicProjectileComponent is not null)
            {
            ...
            if (EnemyReflectionCache.ProjectileLauncherCollider is not null)
            {
            ...
            if (EnemyReflectionCache.ProjectileLauncherReloadOffset is not null)
            {
...
class WeaponEquipPatch
{
    static void Postfix(Weapon __instance, WeaponManager wm)
    {
        try
        {
            ...
            if (wm is not null && wm.healthSystem is not null)
            {
