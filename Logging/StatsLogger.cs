...
{
    private static readonly Lazy<StatsLogger> _lazy = new(() => new StatsLogger());
    ...
    private StatsLogger()
    {
        try
        {
            string gameDirectory = Path.GetDirectoryName(UnityEngine.Application.dataPath) ?? Environment.CurrentDirectory;
    ...
    private string FormatGameStats(GameStatsSnapshot statsSnapshot)
    {
        ...
        if (statsSnapshot.ActivePlayers is not null && statsSnapshot.ActivePlayers.Any())
        {
            ...
            foreach (var playerEntry in sortedPlayers)
            {
                ...
                if (playerData.DeathsPerMap is not null && playerData.DeathsPerMap.Any())
                {
                ...
                if (playerData.WeaponHits is not null && playerData.WeaponHits.Any())
                {
                ...
                if (playerData.EnemyKills is not null && playerData.EnemyKills.Any())
                {
        ...
        if (statsSnapshot.Titles is not null && statsSnapshot.Titles.Count > 0)
        {
        ...
        if (statsSnapshot.MapsPlayed is not null && statsSnapshot.MapsPlayed.Any())
        {
            var totalDeathsPerMap = new Dictionary<string, int>();
            if (statsSnapshot.ActivePlayers is not null)
            {
                foreach (var player in statsSnapshot.ActivePlayers)
                {
                    if (player.Value.DeathsPerMap is not null)
                    {
                        foreach (var mapEntry in player.Value.DeathsPerMap)
                        {
                            totalDeathsPerMap[mapEntry.Key] = totalDeathsPerMap.TryGetValue(mapEntry.Key, out int existing) ? existing + mapEntry.Value : mapEntry.Value;
                        }
                    }
                }
            }
    ...
    private string FormatTimeSpan(TimeSpan timeSpan)
    {
        return timeSpan.TotalDays >= 1
            ? $"{timeSpan.Days}d {timeSpan.Hours:00}:{timeSpan.Minutes:00}:{timeSpan.Seconds:00}"
