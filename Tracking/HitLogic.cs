...
public static class HitLogic
{
    ...
    private static readonly HashSet<int> recentlyKilledEnemies = [];
    private static float lastEnemiesCleanupTime = 0f;
    private static readonly object recentlyKilledLock = new();
    private static readonly HashSet<int> recentlyKilledPlayers = [];
    private static float lastPlayersCleanupTime = 0f;
    private static readonly object recentlyKilledPlayersLock = new();

    private static readonly Dictionary<int, int> rollerBrainHealthTracker = [];
    private static readonly object rollerBrainLock = new();

    private static readonly Dictionary<int, PlayerInput> discOwnerTracker = [];
    private static readonly object discOwnerLock = new();

    private static readonly HashSet<string> namesOfTargetsThatCanDie =
    [
        "Wasp(Clone)",
        "Wasp Shielded(Clone)",
        ...
        private static readonly Dictionary<string, string> enemyDisplayNames = new()
    {
        { "Wasp(Clone)", "Wasp" },
    ...
    public static void TryRecordKill(GameObject target, PlayerInput player, string weaponName)
    {
        ...
        if (enemyBrain is not null)
        {
            RollerBrain rollerBrain = enemyBrain.GetComponentInParent<RollerBrain>();
            if (rollerBrain is not null && !WillRollerStrutKillCauseRollerBrainDeath(rollerBrain))
            {
                return;
            }
    ...
    public static bool WillRollerStrutKillCauseRollerBrainDeath(RollerBrain rollerBrain)
    {
        ...
        lock (rollerBrainLock)
        {
            if (!rollerBrainHealthTracker.ContainsKey(rollerBrainId))
            {
                ...
                if (rollerBrain.struts is not null)
                {
    ...
    {
        if (discProjectile is null || owner is null) return;
    ...
    {
        if (discProjectile is null) return null;
    ...
    {
        if (discProjectile is null) return;
    ...
    public static bool IsTargetImmune(GameObject target)
    {
        ...
        if (enemyHealthSystem is not null && ReflectionCache.EnemyImmuneTimeField is not null)
        {
        ...
        if (spiderHealthSystem is not null && ReflectionCache.SpiderImmuneTimeField is not null)
        {
    ...
    public static bool HasActiveShield(GameObject target)
    {
        ...
        if (enemyHealthSystem is not null)
        {
            return enemyHealthSystem.shield is not null && enemyHealthSystem.shield.activeInHierarchy;
        }
    ...
    {
        if (target is null || playerInput is null) return;
        SpiderHealthSystem spiderHealth = target.GetComponentOrParent<SpiderHealthSystem>();

        if (spiderHealth is not null && spiderHealth.rootObject is not null)
        {
            PlayerInput victimPlayerInput = spiderHealth.rootObject.GetComponentInParent<PlayerInput>();
            if (victimPlayerInput is not null && victimPlayerInput != playerInput)
            {
    ...
    {
        if (target is null || playerInput is null) return;
        string targetName = target.GetComponentInParent<SpiderHealthSystem>() is not null
