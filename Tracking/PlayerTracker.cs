...
{
    private static readonly Lazy<PlayerTracker> _lazy = new(() => new PlayerTracker());
    public static PlayerTracker Instance => _lazy.Value;

    private readonly Dictionary<PlayerInput, PlayerData> activePlayers = [];

    private readonly Dictionary<ulong, PlayerInput> playerIds = [];
    ...
    private static readonly float playerCacheRefreshInterval = 60f; // Refresh every 60 seconds
    ...
    public void RecordPlayerDeath(SpiderHealthSystem spiderHealth)
    {
        ...
        try
        {
            ...
            if (spiderHealth.rootObject is not null)
            {
            ...
            if (playerInput is not null && activePlayers.TryGetValue(playerInput, out PlayerData data))
            {
                data.Deaths++;
                data.KillStreak = 0;
                ...
                data.DeathsPerMap[currentMap] = data.DeathsPerMap.TryGetValue(currentMap, out int deaths) ? deaths + 1 : 1;
            }
            else
            {
    ...
    public void UndoPlayerDeath(SpiderHealthSystem spiderHealth)
    {
        ...
        try
        {
            ...
            if (playerInput is not null && activePlayers.TryGetValue(playerInput, out PlayerData data))
            {
                ...
                string currentMap = GameSessionManager.Instance.CurrentMapName;
                if (data.DeathsPerMap.TryGetValue(currentMap, out int mapDeaths) && mapDeaths > 0)
                {
                    if (mapDeaths == 1)
                        data.DeathsPerMap.Remove(currentMap);
                    else
                        data.DeathsPerMap[currentMap] = mapDeaths - 1;
                }
    ...
    {
        if (player is not null && activePlayers.TryGetValue(player, out PlayerData data))
        {
            data.Kills++;
            data.KillStreak++;
            ...
            if (spider is not null)
            {
                ...
                if (stabilizer is not null && !stabilizer.grounded)
                {
    ...
    {
        if (player is not null && spider is not null && activePlayers.TryGetValue(player, out PlayerData data))
        {
            if (spider.bodyRigidbody2D is not null)
            {
    ...
    public static PlayerController[] GetCachedPlayerControllers()
    {
        lock (playerCacheLock)
        {
            ...
            if (cachedPlayerControllers is null || currentTime - lastPlayerCacheUpdate > playerCacheRefreshInterval)
            {
    ...
    public static PlayerInput FindPlayerInputByPlayerId(ulong playerId)
    {
        ...
        {
            if (controller is not null && (ulong)controller.playerID.Value == playerId)
            {
