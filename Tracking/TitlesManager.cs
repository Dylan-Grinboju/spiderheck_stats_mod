...
public class TitleBuilder(StatLeaders leaders)
{
    private readonly StatLeaders _leaders = leaders;
    private KeyValuePair<PlayerInput, PlayerTracker.PlayerData> _primaryLeader;
    private readonly HashSet<string> _requirements = [];
...
{
    private static readonly Lazy<TitlesManager> _lazy = new(() => new TitlesManager());
    public static TitlesManager Instance => _lazy.Value;
    private const int MaxDisplayedTitles = 8;

    private List<TitleEntry> currentTitles = [];
    ...
    public void CalculateAndStoreTitles(GameStatsSnapshot snapshot)
    {
        ...
        if (snapshot?.ActivePlayers is null || snapshot.ActivePlayers.Count <= 1)
        {
    ...
    private void RedistributeTitlesByCountGap(List<TitleEntry> selectedTitles, List<TitleEntry> remainingTitles, List<TitleEntry> allTitles)
    {
        ...
        while (iterations++ < maxIterations)
        {
            ...
            if (titleToRemove is null || replacementTitle is null)
            {
                return;
            }
